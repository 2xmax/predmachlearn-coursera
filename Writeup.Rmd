...
In order to achieve the best accurancy without requirement of good model interpretaion the best choice will be random forests algorithm (at least among RF, GBM and SVM. Look for [code.R](https://github.com/2xmax/predmachlearn-coursera/blob/master/code.R) for details).
```{r}
# predictor <- train(classe ~ ., data = train, method="gbm") # Stochastic Gradient Boosting - 95.99% accurancy rate
#predictor <- train(classe ~ ., data = train, method="svmRadial") # Support Vector Machines - 88.97% accurancy rate
